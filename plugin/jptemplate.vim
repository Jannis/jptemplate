...
function! s:ProcessTemplate(info, template)
  ...
  while 1
    ...
    if start < 0
      ...
      let [name, value] = <SID>ParseExpression(expr)
  ...
  for expr in keys(reserved)
    let [name, value] = <SID>ParseExpression(expr)
    let replacement = <SID>EvaluateReservedVariable(name, value, variables)
  ...
  let [insertedLines, startColumn, endColumn] = <SID>ExpandTemplate(a:info, a:template)

  " Update the cursor position and return to insert mode
  call <SID>UpdateCursorPosition(insertedLines, endColumn)
endfunction
...
function! s:InsertTemplate()
  try
    ...
    let info = <SID>GetTemplateInfo()

    " Load the template file
    let template = <SID>ReadTemplate(info['name'], info['filename'])

    " Do the hard work: Process the template
    call <SID>ProcessTemplate(info, template)
  catch
    " Inform the user about errors
    echoerr g:jpTemplateVerbose ? v:exception . " (in " . v:throwpoint . ")" : v:exception
  endtry
endfunction
...
" Command
command! JPTemplate :call <SID>InsertTemplate()

" Key Mapping
exec 'imap ' . g:jpTemplateKey . ' <Esc>JPTemplate()<Return>'
